...
**2. Часть 2. Создание сети и настройка основных параметров устройства;**
  ...
  * Зададим имя устройства: hostname SW1.
  * Далее, чтобы при ошибочном вводе комманд коммутатор не искал доменный сервер введём комманду: no ip domain-lookup.
  * Установим пароль на консольное соединение: line console 0 и зададим пароль: password cisco. Далее включим запрос пароля: enable. Теперь при подключении через консоль появляется запрос на ввод пароля.
